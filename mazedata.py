...
class MazeBase:
    def __init__(self):
        self.portalPairs: Dict[int, Tuple[Tuple[int, int], Tuple[int, int]]] = {}
        self.homeoffset: Tuple[int, int] = (0, 0)
        self.ghostNodeDeny: Dict[int, Tuple] = {UP: (), DOWN: (), LEFT: (), RIGHT: ()}
    ...
    def denyGhostsAccess(self, ghosts, nodes):
        deny_access = self.addOffset(2, 3)
        nodes.denyAccessList(*(deny_access + (LEFT, ghosts)))
        nodes.denyAccessList(*(deny_access + (RIGHT, ghosts)))

        for direction, values in self.ghostNodeDeny.items():
            for value in values:
                nodes.denyAccessList(*(value + (direction, ghosts)))
